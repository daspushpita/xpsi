...
class HotRegion(ParameterSubspace):
    ...
    def __compute_cellParamVecs(self):
        ...
        if not self.temperature_gradiant:
            self._super_cellParamVecs[...,:-1] *= self['super_temperature']
        else:
            #Insert the interpolated values----------------------------------
            mytheta = self._super_theta
            myphi = self._super_phi
            self._super_cellParamVecs[...,:-1] *= self['mygrid_temperature']
            #----------------------------------------------------------------
